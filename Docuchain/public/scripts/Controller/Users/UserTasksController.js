  ...
  .controller('UserTasksController', [
    ...
    function (
      ...
      $scope.updateStatusWithRemarkToUsers = function (assignedTasks) {
        ...
        // --- FIX START ---
        // The HTML select sets assignedTasks.taskStatus to a stringified object like:
        // '{"taskStatusId":5,"taskStatus":"Rejected"}'
        // We parse it here to extract the ID and String separately before sending.
        var finalTaskStatusId = assignedTasks.taskStatus;
        var finalTaskStatusString = assignedTasks.taskStatus;

        try {
          // If it's a stringified JSON object, parse it
          if (
            typeof assignedTasks.taskStatus === 'string' &&
            assignedTasks.taskStatus.indexOf('{') !== -1
          ) {
            var statusObj = JSON.parse(assignedTasks.taskStatus);
            finalTaskStatusId = statusObj.taskStatusId;
            finalTaskStatusString = statusObj.taskStatus;
          }
        } catch (e) {
          console.log('Status parse fallback', e);
        }

        var taskData = {
          createdBy: $scope.userId,
          taskStatusId: parseInt(finalTaskStatusId), // Sent safely as a Number/Long
          taskStatus: finalTaskStatusString, // Sent safely as a String
          taskId: assignedTasks.taskId,
          userRemarks: assignedTasks.userRemarks,
          loginId: $scope.userId,
        };
